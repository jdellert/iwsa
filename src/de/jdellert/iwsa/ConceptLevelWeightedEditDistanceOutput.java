...
public class ConceptLevelWeightedEditDistanceOutput {
	public static void main(String[] args) {
		try {
			...
			{
				double pmiScore = - Math.log((cognatePairCorrespondenceCounts[symbolPairID] + 1.0)/(randomPairCorrespondenceCounts[symbolPairID] / 100.0 + 1));
				System.err.println(symbolTable.toSymbol(symbolPairID / symbolTable.getSize()) + "\t" + symbolTable.toSymbol(symbolPairID % symbolTable.getSize()) + "\t" + pmiScore);
			}
		} catch (IOException e) {
